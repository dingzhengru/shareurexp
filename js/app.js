...
var mainApp = angular.module('mainApp', ['modalsApp',
                                         'userApp',
                                         'ui.router',
                                         'ui.bootstrap', 
                                         'ngAnimate', 
...
.config(function($stateProvider, $urlRouterProvider) {
    $stateProvider
        .state('home', {
            ...
            controller: function($scope, $window, $interval) {
                ...
                $scope.scrollChangeNav = function(pageSize, totalItems) {
                    ...
                    // off then on
                    $scope.$on('$destroy', () => angular.element($window).off('scroll', changeNav));
        ...
        .state('articles', {
            ...
            controller: function($scope, $firebaseArray, $state, $stateParams, $window, $timeout) {
                ...
                $scope.searchArticleByTag = (tagName, event) => { 
                ...
                $scope.pageSize = 4;
                $scope.maxSize = 5;
                $scope.articlesArray.$loaded().then((articles) => $scope.scrollPagination($scope.pageSize, articles.length));
        ...
        .state('article', {
            ...
            controller: function($scope, $state, $stateParams, $firebaseObject, $firebaseArray, $window, $timeout) {
                $scope.setPageTitle('文章內容');
                $scope.articleID = $stateParams.articleID;
                $scope.article = $firebaseObject(articlesRef.child($scope.articleID));

                $scope.isPushed = (article, UserID) => { 
                    try { return article.pushUserIDs.indexOf(UserID) > -1; } catch(error) {return false;}
                };
                $scope.pushArticle = (article, pushUserID) => {
                    if(!$scope.isAuth()) {
                ...
                $scope.postComment = (article, comment) => {
                    ...
                    article.$save(article).then(() => {
                        // show success -> hide success
                        $scope.isPostArticleComment = true;
                        $timeout(() => $scope.isPostArticleComment = false, 1000);
                    });
                ...
                $scope.getPopverContent = (article) => {
                    try {
                        if($scope.isAuthSelf(article.creator.uid))
                            return '不能推自己的文章';
                        if($scope.isPushed(article, $scope.getAuth().uid))
                            return '已經推過了';
                        return ''
                    } catch (e) { return '' }
                }
        ...
        .state('tags', {
            ...
            controller: function($scope, $firebaseArray, $state, $timeout, $window) {
                ...
                $scope.pageSize = 24;
                $scope.maxSize = 5;
                $scope.tagsArray.$loaded().then((tags) => $scope.scrollPagination($scope.pageSize, tags.length));
                $scope.scrollPagination = function(pageSize, totalItems) {
                    if(!$scope.isMobile.any()) return;
                ...
                $scope.searchArticleByTags = (tags, event) => {
                    let tagsNames = tags.map((tag) => { return tag.name });
                    let searchArticleString = tagsNames.join(' ');
                    $state.go('articles', {searchArticle: searchArticleString});
                }
                
                $scope.addTag = (tag) => {
                    if($scope.arrayObjectIndexOf($scope.tagSelectedList, tag, 'name') == -1)
                        $scope.tagSelectedList.push(tag);
                }
                
                $scope.removeTagByIndex = (index) => {
                    $scope.tagSelectedList.splice(index, 1);
                }
                $scope.removeTagByTag = (tag) => {
                    let index = $scope.arrayObjectIndexOf($scope.tagSelectedList, tag, 'name')
                    $scope.tagSelectedList.splice(index, 1);
                }
                
                $scope.isTagSelected = (tag) => {
        ...
        .state('post-article', {
            ...
            controller: function($scope, $state, $firebaseArray, $firebaseObject, $uibModal, $timeout, $window) {
                ...
                $scope.taOptions.toolbar = [
                    ...
                    ['insertLink', 'insertImage', 'insertVideo', 'charcount']
                ]
                ...
                $scope.postArticle = (newArticle) => {
                    ...
                    newArticle.departments.forEach((department) => {
                    ...
                    article.tags.forEach((tag) => {
                        article.keywords.push(...tag.keywords);
                    ...
                    $scope.articlesArray.$add(article).then(function(ref) {
                        $scope.isPost = true;
                        $timeout(() => $state.go('article', { 'articleID': ref.key() }), 500);
                    });
                }
            }
        })
        .state('edit-article', {
            ...
            templateUrl: 'templates/post-article.html',
            controller: function($scope, $state, $stateParams, $firebaseObject, $firebaseArray, $uibModal, $timeout, $window) {
                ...
                let articleObject = $firebaseObject(articlesRef.child($scope.articleID));
                
                // 處理style跟tag
                articleObject.$loaded().then((article) =>  {
                    ...
                    $scope.stylesArray.$loaded().then(function(styles){
                        ...
                        $scope.article.tags = $scope.article.tags.filter((tag) => { // 從tags刪除 style
                            return tag.order == 2;
                        })
                    });
                });
                ...
                $scope.postArticle = (article) => {
                    ...
                    article.departments.forEach((department) => {
                        article.tags.push(department);
                    })
                    ...
                    article.$save(article).then(function(ref) {
                        $scope.isEdit = true;
                        $timeout(() => $state.go('article', { 'articleID': ref.key() }), 200);
                    });
                }
            }
        })
...
.controller('MainController', function($scope, $location, $firebaseArray, $firebaseObject, $state, $uibModal, $rootScope, $window, $timeout, $uibPosition) {
    ...
    $scope.getUserProfile = function() {
        ...
        let profile = $scope.getAuth().facebook || $scope.getAuth().google || null;
        ...
        let superUserIDs = [
            'google:118138672712062784598', 
            'facebook:1178589075493805', 
            'fTSce15UEbXyjCI0Gk7tsXKzGT83'
        ]
        
        if(superUserIDs.indexOf(profile.uid) >= 0)
    ...
    $scope.openLoginModal = function(size, state, params) {
    ...
    $scope.openReportArticleModal = function(size, article) {
        let modalInstance = $uibModal.open({
            animation: true,
            size: size,
            ...
            resolve: {
                ...
                user: () => { return $scope.getUserProfile(); }
            }
        });
    }
    ...
    $scope.goAuthState = function(state, params) {
        if($scope.isAuth())
            ...
            $scope.openLoginModal('sm', state, params || {});
    }
    ...
    $scope.changeStyle = function(stylesArray, style) {
        ...
        // 判斷是否全都沒勾
        let isAllFalse = stylesArray.every((style) => { return !style.isSelected })
        if(isAllFalse) {
    ...
    $scope.addNotification = function(userID, notification) {
        $firebaseObject(usersRef.child(userID)).$loaded().then((user) => {
    ...
    $scope.readAllNotifications = function(user, notReadCount) {
        ...
        $timeout(() => {
            user.notifications.forEach((notification) => notification.isRead = true);
    ...
    $scope.isMobile = {
        ...
        Windows: () => {  return !!navigator.userAgent.match(/IEMobile/i) || !!navigator.userAgent.match(/WPDesktop/i); },
        any: () => { return ($scope.isMobile.Android() || $scope.isMobile.BlackBerry() || $scope.isMobile.iOS() || $scope.isMobile.Opera() || $scope.isMobile.Windows()); }
    };
    ...
    $firebaseArray(researchRef).$loaded().then(() => $scope.isLoaded = true);
    
    // $scope.user
    if($scope.isAuth())
        $scope.user = $firebaseObject(usersRef.child($scope.getAuth().uid));
    ...
    $scope.orderOptions = [
        {name: '發表時間較新', key: 'created',           reverse: true },
        {name: '推較多',       key: 'pushUserIDs.length || 0',reverse: true },
        {name: '留言較多',       key: 'comments.length || 0',   reverse: true },
        {name: '發表時間較舊', key: 'created',           reverse: false },
        {name: '推較少',       key: 'pushUserIDs.length || 0',reverse: false },
        {name: '留言較少',       key: 'comments.length || 0',   reverse: false }
    ]
