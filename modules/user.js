...
/*
    ...
    global Firebase
*/
var userApp = angular.module('userApp', ['ui.router', 'ui.bootstrap', 'firebase']).config(function ($stateProvider, $urlRouterProvider) {
    $stateProvider.state('user', {
        url: '/user/:userID',
        templateUrl: 'templates/user/user.html',
        controller: function controller($scope, $state, $stateParams, $firebaseArray, $firebaseObject, $location, $timeout, $window) {
            ...
            $scope.searchUserArticleByTag = function (tagName) {
                return $scope.searchUserArticle = tagName;
            };
            ...
            $scope.scrollPagination = function (pageSize, totalItems) {
                ...
                var paginationOnScroll = function paginationOnScroll() {
                    $timeout(function () {
                        if ($window.innerHeight + $window.scrollY >= document.body.offsetHeight - $('footer').height() - $('.user-articles-table').height() && $scope.articlesPageSize < totalItems) {
        ...
        controller: function controller($scope, $state, $firebaseArray) {
            ...
            var articlesArray = $firebaseArray(articlesRef.orderByChild('creator'));
            articlesArray.$loaded().then(function (articles) {
                angular.forEach(articles, function (article, key) {
                    if (article.creator.uid == $scope.userID) $scope.articlesArray.push(article);
                });
                // 滑動換頁
        ...
        controller: function controller($scope, $state, $firebaseArray) {
            ...
            $scope.articlesArray = [];
            var articlesArray = $firebaseArray(articlesRef);

            articlesArray.$loaded().then(function (articles) {
                angular.forEach(articles, function (article, key) {
                    if ($scope.user.pushedArticleIDs.indexOf(article.$id) > -1) $scope.articlesArray.push(article);
                });
                // 滑動換頁
                $scope.scrollPagination($scope.pageSize, articles.length);
            });
